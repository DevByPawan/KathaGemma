...
import { startNewStory, getChildStories } from '@/services/story';
  ...
  useEffect(() => {
    const fetchLatestStory = async () => {
      try {
        ...
        console.error('Failed to load child stories:', error);
      }
    };
    fetchLatestStory();
  }, [profile]);
    ...
    <div className="space-y-5">
      ...
      >
        ...
        <div className="px-5 pb-5 space-y-3.5">
          ...
          >
            <p className="text-[9px] font-black uppercase text-indigo-300 tracking-wider mb-2">Switch Learner Profile</p>
            <div className="flex flex-wrap gap-2">
              {[
                { id: '11111111-2222-3333-4444-555555555555', name: 'Aarav', emoji: '🐼' },
                { id: '22222222-2222-3333-4444-555555555555', name: 'Leo', emoji: '🦊' },
                { id: '33333333-2222-3333-4444-555555555555', name: 'Meera', emoji: '🐘' },
                { id: '44444444-2222-3333-4444-555555555555', name: 'Raju', emoji: '🦜' },
                { id: '55555555-2222-3333-4444-555555555555', name: 'Mili', emoji: '🐼' }
              ].map((child) => {
                const isActive = (profile?.name || 'Aarav').toLowerCase() === child.name.toLowerCase();
                return (
                  <button
                    key={child.id}
                    type="button"
                    onClick={async () => {
                      localStorage.setItem('katha_child_id', child.id);
                      localStorage.setItem('katha_avatar', child.name === 'Aarav' ? 'Panda' : child.name === 'Leo' ? 'Fox' : child.name === 'Meera' ? 'Elephant' : child.name === 'Raju' ? 'Parrot' : 'Panda');
                      await reloadProfile();
                    }}
                    className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full border transition-all active:scale-95 text-xs font-black cursor-pointer
                      ${isActive
                        ? 'border-indigo-400 bg-indigo-500 text-white shadow-sm ring-2 ring-indigo-400/20'
      ...
      <motion.section variants={fadeUp} initial="hidden" animate="visible" custom={4}>
        ...
        {latestStory ? (
          <motion.div
            ...
            onClick={() => navigate(`/story?id=${latestStory.id}`)}
            role="button"
            tabIndex={0}
            onKeyDown={(e) => e.key === 'Enter' && navigate(`/story?id=${latestStory.id}`)}
            aria-label={`Continue story: ${latestStory.title}`}
          >
            {/* Top gradient bar */}
                ...
                <div className="flex-1 space-y-1">
                  <div className="flex items-center gap-2">
                    <span className={`text-[10px] font-black uppercase tracking-widest px-2 py-0.5 rounded-full
                      ${latestStory.status === 'COMPLETED'
                        ? 'text-emerald-300 bg-emerald-500/15'
                        : 'text-secondary bg-secondary/15'
                      }`}
                    >
                      {latestStory.status === 'COMPLETED' ? 'Completed' : 'In Progress'}
                    </span>
                  </div>
                  <h3 className="text-lg font-black text-white leading-tight">{latestStory.title}</h3>
                  <p className="text-xs text-zinc-400 font-medium">
                    {latestStory.status === 'COMPLETED'
                      ? 'Completed all 5 chapters!'
                      : `Chapter ${latestStory.generatedStory.length} of 5 · Tap to read next`}
                  </p>
                </div>
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between text-xs">
                  ...
                  <span className="text-primary font-black">
                    {Math.min(100, Math.floor((latestStory.generatedStory.length / 5) * 100))}%
                  </span>
                </div>
                <ProgressBar progress={Math.min(100, (latestStory.generatedStory.length / 5) * 100)} />
              </div>
            </div>
          </motion.div>
        ) : (
          <motion.div
            ...
            aria-label="No story in progress. Start new adventure."
          >
            <div className="p-6 text-center space-y-2">
              <BookOpen className="size-7 text-zinc-500 mx-auto opacity-70 animate-pulse" />
              <div className="space-y-0.5">
                <h3 className="text-sm font-black text-zinc-400">No story in progress</h3>
                <p className="text-[11px] text-zinc-500 font-semibold">Tap "New Adventure" below or start reading a tale!</p>
              </div>
            </div>
          </motion.div>
        )}
